...
function createSummaryTable() {
    ...
    clans.forEach(clan => {
        ...
        const sortedPlayersInClan = playersInClan.sort((a, b) => b[8] - a[8]).slice(0, 50); // Ordina i giocatori per coppe

        let clanScore = 0;
        sortedPlayersInClan.forEach((row, index) => {
            let multiplier = 0;
            if (index === 0) multiplier = 0.5;
            else if (index < 10) multiplier = 0.4;
            else if (index < 20) multiplier = 0.3;
            else if (index < 30) multiplier = 0.2;
            else if (index < 40) multiplier = 0.1;
            else if (index < 50) multiplier = 0.05;
            clanScore += parseFloat(row[8]) * multiplier; // Usa parseFloat per gestire numeri decimali
        });

        clanScore = Math.round(clanScore); // Arrotonda il punteggio totale alla fine
